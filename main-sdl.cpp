...
bool mouse_left_down, mouse_left_just_down, mouse_left_down_previous = 0;
...
void map_load(Map* map, const char* file_path) {
    map_unload(map);
    ...
    while (fgets(line, 1001, file)) {
        if (map_width == -1) map_width = (int)strlen(line) - 1;
        line[map_width] = '\0';

        map_height += 1;

        map_matrix = (char**)realloc(map_matrix, sizeof(char*) * map_height);
        map_matrix[map_height - 1] = (char *) calloc(map_width + 1, sizeof(char));
...
int main2(int argc, char* argv[]) {
    ...
    while (events()) {
        ...
        x1 += dt * 0.02;
        if (x1 > 150) x1 = 0;
        SDL_Rect button1 = { .x = (int)x1,.y = 10,.w = 50,.h = 50 };
        ...
        x2 += dt * 0.08;
        if (x2 > 150) x2 = 0;
        SDL_Rect button2 = { .x = (int)x2,.y = 10 + 50 + 10,.w = 50,.h = 50 };
        //int button_color[] = { 0, 50, 50 };
        SDL_SetRenderDrawColor(renderer, button_color[0], button_color[1], button_color[2], 255);
